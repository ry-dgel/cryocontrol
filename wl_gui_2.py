...
def fit_scan(spectrum):
    ...
    lengths.sort()
    for i in range(wlfitter.settings['n_peaks'] - 1):
        data[f'lengths{i+1}'].append(lengths[i])
        data[f'errors{i+1}'].append(errors[i])
    for i in range(wlfitter.settings['n_peaks'] - 1,4):
    ...
    dpg.set_value("fft_init", [list(wlfitter.lengthu), list(wlfitter.fit.init_fit)])
    ...
    dpg.set_value("length1", [data['times'],data['lengths1']])
    dpg.set_value("length2", [data['times'],data['lengths2']])
    dpg.set_value("length3", [data['times'],data['lengths3']])
    dpg.set_value("length4", [data['times'],data['lengths4']])
...
def set_fitter(*args):
    ...
    wlfitter.settings['n_peaks'] = wl_tree["Fitting/N Cavity Peaks"] + 1
